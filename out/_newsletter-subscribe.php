<?php
/*
 * Petrichor v1.0 - simple newsletter subscription script
 * (c) Web factory Ltd
**/
...
**/
  $myEmail = 'contato@peopleinteractive.com.br';
...
/**** DO NOT EDIT BELOW THIS LINE ****/
  ...
  if (fwrite($file, $emailAddress . ' - ' . $name . PHP_EOL) == strlen($emailAddress . ' - ' . $name . PHP_EOL)) {
    ...
    if (preg_match('/^[^@]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$/', trim($myEmail))) {
        ...
        $message = 'O seguinte contato se cadastrou no site solicitando retorno:' . PHP_EOL . $name . ' - ' . $emailAddress;
        @mail($myEmail, 'Prospect Site People Interactive', $message, $headers);
    }
